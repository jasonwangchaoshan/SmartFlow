...
uint8_t TX_test_buff[16] = "TX_descr_str";
...
void char2_notify_buff_handle(esp_gatt_if_t gatts_if, uint16_t conn_id) 
{
	//if (is_notify==1) 
	{
	ESP_LOGI(GATTS_TAG, "-----char2_notify_buff_handle ------\n");

	esp_ble_gatts_send_indicate(gatts_if, conn_id, gl_char[1].char_handle,10,&TX_test_buff,false);
...
void RX_char_write_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    if (strncmp((const char *)gl_char[0].char_val->attr_value,"getd",6)==0) {//比较字符串，若相等则控制LED灯
    //	gpio_set_level(LED_PIN,HIGH);
    //	led_stat=1;
    	strcpy((const char *)TX_test_buff,"                ");
    	strcpy((const char *)TX_test_buff,"LED ON");
    	char2_notify_buff_handle(gatts_if, param->write.conn_id);//发送数据    	
    	    	
    } else if (strncmp((const char *)gl_char[0].char_val->attr_value,"LED OFF",7)==0) {//比较字符串，若相等则控制LED灯
    //	gpio_set_level(LED_PIN,LOW);
    	//led_stat=0;
    	strcpy((const char *)TX_test_buff,"                ");
    	strcpy((const char *)TX_test_buff,"LED OFF");
    	char2_notify_buff_handle(gatts_if, param->write.conn_id);//发送数据    	
    } else if (strncmp((const char *)gl_char[0].char_val->attr_value,"LED SWITCH",10)==0) {//比较字符串，若相等则控制LED灯
    //	led_stat=1-led_stat;
    //	gpio_set_level(LED_PIN,led_stat);
    
    } else {
    	//char2_notify_handle(gatts_if, param->write.conn_id);//发送数据
    }
        	//char2_notify_buff_handle(gatts_if, param->write.conn_id);//发送数据    	
